...
bool ESKF<S>::Predict(const IMU& imu) {
    ...
    // Q2 modification
    // VecT dp = dx_.template segment<3>(0);
    // VecT dv = dx_.template segment<3>(3);
    // VecT dtheta = dx_.template segment<3>(6);
    // VecT dbg = dx_.template segment<3>(9);
    // VecT dba = dx_.template segment<3>(12);
    // VecT dg = dx_.template segment<3>(15);
    

    // dx_.template segment<3>(0) += dv * dt;
    // dx_.template segment<3>(3) += -R_.matrix() * SO3::hat(imu.acce_ - ba_) * dt * dtheta - R_.matrix() * dt * dba + Mat3T::Identity() * dt * dg;
    // dx_.template segment<3>(6) += SO3::exp(-(imu.gyro_ - bg_) * dt).matrix() * dtheta - Mat3T::Identity() * dbg;
